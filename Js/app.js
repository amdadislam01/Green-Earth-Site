...
const getAllPlants = async () => {
  toggleLoading("loading", true); // loader show
  ...
  plants.forEach(plant => {
    ...
    card.innerHTML = `
      <div class="bg-white rounded-lg shadow p-4">
          ...
          <button class="mt-4 w-full bg-green-600 hover:bg-white hover:border cursor-pointer hover:border-gray-400 hover:text-black font-bold text-lg text-white py-3 rounded-full">
              Add to Cart
          </button>
      </div>
    `;
    ...
    categoryBtn.addEventListener('click', (e) => {
      ...
      setTimeout(() => {
        if (filteredPlants.length > 0) {
          ...
          console.log("No plants found for this category");
        }
        toggleLoading("loading", false); 
      }, 800);
    });
